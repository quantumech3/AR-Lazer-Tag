...
public class CP2130SpiDevice extends UsbSpiDevice
{
    ...
    private int setControlCommandOut(int request, int value, int index, byte[] data)
    {
        ...
        int response = connection.controlTransfer(BM_REQ_HOST_2_DEVICE, request, value, mInterface.getId(), data, dataLength, com.felhr.usbserial.UsbSpiDevice.USB_TIMEOUT);
    ...
    private byte[] setControlCommandIn(int request, int value, int index, int length)
    {
        ...
        int response = connection.controlTransfer(BM_REQ_DEVICE_2_HOST, request, value, mInterface.getId(), data, length, com.felhr.usbserial.UsbSpiDevice.USB_TIMEOUT);
        Log.i(CLASS_ID,"Control Transfer Response: " + String.valueOf(response));
        return data;
    }
}
